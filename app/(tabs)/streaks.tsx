...
const StreaksScreen = () => {
  ...
  const [completedHabits, setCompletedHabits] = useState<HabitsCompletion[]>(
    []
  );
  const { user } = useAuth();
  useEffect(() => {
  ...
  const getStreakData = (habitId: string): StreakData => {
    ...
    let streak = 0;
    let bestStreak = 0;
    let total = habitsCompletions.length;
    //
    let lastDate: Date | null = null;
    let currentStreak = 0;

    habitsCompletions?.forEach((c) => {
  ...
  return (
    <View style={styles.container}>
      ...
      {habits.length === 0 ? (
        <View>
          <Text variant="bodyLarge">No habits yet. Add your first Habit</Text>
        </View>
      ) : (
        <ScrollView
...
const styles = StyleSheet.create({
  ...
  badge2: {
    backgroundColor: "#c0c0c0", //gold
  },
  badge3: {
    backgroundColor: "#cd7f32", //gold
  },
  rankingBadgeText: {
    fontWeight: "bold",
    color: "#fff",
